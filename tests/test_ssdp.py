...
from provider.ssdp import _MX_MAX_SECONDS, SSDPAdvertiser, _parse_mx_delay
...
async def test_shared_ssdp_port_failure_is_setup_error(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("provider.ssdp.socket.socket", lambda *_args: next(sockets))
    ...
    monkeypatch.setattr("provider.ssdp.asyncio.get_running_loop", lambda: _Loop())
