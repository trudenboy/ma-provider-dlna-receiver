...
# Dev environment setup for the dlna_receiver provider.
set -euo pipefail
...
if [ -n "$WORKSPACE" ]; then
  ...
  LINK="$WORKSPACE/ma-server/music_assistant/providers/dlna_receiver"
  TARGET="$(python3 -c "import os; print(os.path.relpath('$REPO_ROOT/provider/', '$(dirname "$LINK")'))")"
  rm -f "$LINK"
  ln -s "$TARGET" "$LINK"
  echo "Symlink: dlna_receiver → $TARGET"
  ...
  echo "Provider dlna_receiver linked to workspace. Activate: source $WORKSPACE/.venv/bin/activate"
else
