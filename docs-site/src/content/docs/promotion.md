---
title: Promotion & SEO
description: Visual presentation and SEO settings for the DLNA Receiver repository
---

This document describes how the **DLNA Receiver** repository's visual presentation and search-engine surface are maintained.
...
- **`Music Assistant` badge** — dynamic shields.io endpoint at `https://trudenboy.github.io/ma-provider-tools/badges/dlna_receiver.json`. The `update-ma-version-badges.yml` cron in `ma-provider-tools` refreshes the JSON every 4 hours, showing which MA channel (stable / beta / dev) currently ships the provider and at what MA version.
